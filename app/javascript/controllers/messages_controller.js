  ...
  //  console.log(this.messageThreadTargets);
   this.messageThreadTargets.forEach((messageThread) => {
    messageThread.addEventListener('click', (e) => {
      ...
      fetch(`/messages/${messageThread.dataset.messageThreadId}/messages?other_user_id=${messageThread.dataset.messagesTargetUserId}`, { headers: this.headers }).then(response => response.text()).then(html => Turbo.renderStreamMessage(html));
    });
   });
